...
SYSTEM_PROMPT = (
    ...
    " Assistants should use as few tokens as possible while meeting requirements."
)
...
dataset_name = args.dataset
system_prompt = True if args.sys == 1  else 0
...
elif  dataset_name == 'zwhe99/amc23':
    dataset =  load_dataset(dataset_name)
    TEST_N = 10
    MAX_TOKENS = tok_limit
    TEST_TEMPERATURE = 0.6
...
elif dataset_name ==  'math-ai/olympiadbench':
    dataset = load_dataset(dataset_name)
    TEST_N = 5
    MAX_TOKENS = tok_limit
    TEST_TEMPERATURE = 0.6
    MAX_TEST_SAMPLES = 674


addition_dataset = ['zwhe99/amc23','math-ai/olympiadbench']
...
def get_scores(ds, outputs, save_file_name=None):
    ...
    for input, output in zip(ds, outputs):
        ...
        if dataset_name in addition_dataset:
            gold = '\\boxed{' + str(input[ANSWER_KEY])  + "}"
        else:
...
def evaluate_model(model_name):
    test_prompts = []
    model = LLM(model_name, tokenizer=model_name, gpu_memory_utilization=0.8, tensor_parallel_size=2)    
    ...
    for x in test_ds:
        ...
        },
        {
            ...
            # "content": x[QUESTION_KEY] + " " + "Please reason step by step, and put your final answer within \\boxed{}." 
